...
public class Game implements Runnable
{
    ...
    private MouseManager mouseManager;  /*!< Referinta catre obiectul care gestioneaza click-urile de la mouse din partea utilizatorului */
    ...
    /* getter pentru MenuState care returneaza canvasul */
    ...
    private void InitGame()
    {
        ...
        /* obținem Canvas-ul UNIC pe care desenam */
        Canvas canvas = wnd.GetCanvas();
        canvas.setFocusable(true);           // pentru a putea primi focus
        canvas.requestFocus();               // îl cerem imediat

        /* input managers */
        keyManager   = new KeyManager();
        mouseManager = new MouseManager();

        canvas.addKeyListener(keyManager);         //  tastatura PE Canvas
        canvas.addMouseListener(mouseManager);     //  mouse   PE Canvas
        canvas.addMouseMotionListener(mouseManager);

        /* refLink */
        refLink = new RefLinks(this);
        refLink.SetMouseManager(mouseManager);

        /* Implementarea camerei */
        // int mapPxW = Tile.TILE_WIDTH * map.DEFAULT_WIDTH;
        GameCamera cam = new GameCamera(refLink, 0, 0);
        refLink.SetGameCamera(cam);


        /* inițializări suplimentare */
        Assets.Init();

        playState     = new PlayState(refLink, true);
...
// PAS 1: Curăță tot fundalul cu imagine sau culoare
        if (backgroundImage != null) {
            g.drawImage(backgroundImage, 0, 0, wnd.GetWndWidth(), wnd.GetWndHeight(), null);
        } else {
            g.setColor(Color.BLACK);  // fallback dacă nu se încarcă imaginea
            g.fillRect(0, 0, wnd.GetWndWidth(), wnd.GetWndHeight());
        }

// PAS 2: Desenează starea activă (joc, meniu, etc)
        if(State.GetState() != null)
        {
