...
public abstract class Character extends Item
{
    ...
    public void MoveX()
    {
        ...
        int mapWidth = refLink.GetMap().GetWidth() * Tile.TILE_WIDTH;

        if (nextX < 0) {
            x = 0;
        } else if (nextX + bounds.width > mapWidth) {
    ...
    public void MoveY()
    {
        ...
        int mapHeight = refLink.GetMap().GetHeight() * Tile.TILE_HEIGHT;

        if (nextY < 0) {
            y = 0;
        } else if (nextY + bounds.height > mapHeight) {
